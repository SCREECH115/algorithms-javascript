...
* Purpose of the task
*------------
* Write a function that will count the duration of the course divided into lessons prepared by Adam, Marcin and Przemek.

*
* The function is supposed to return a sentence:
* "The Master JavaScript course lasts x hours and y minutes.
    Adam module: ... hours ... minutes
    Przemek's module: ... hours ... minutes
    Martin module: ... hours ... minutes
"
*/
...
const teachers = () => {
  ...
  for (let i = 0; i < lessons.length; i++) {
    courseModules.push(lessons[i]);

    if (lessons[i].name.includes("1x")) {
      adamModules.push(lessons[i]);
    } else if (lessons[i].name.includes("2x")) {
      przemekModules.push(lessons[i]);
    } else if (lessons[i].name.includes("3x")) {
      marcinModules.push(lessons[i]);
    }
  }

  console.log(`The cours lasts ${calculateTeacherTime(courseModules)}`);
  console.log(`Adam's module lasts ${calculateTeacherTime(adamModules)}`);
  console.log(`Przemek's module lasts ${calculateTeacherTime(przemekModules)}`);
  console.log(`Marcin's module lasts ${calculateTeacherTime(marcinModules)}`);
};

const calculateTeacherTime = (teacherModule) => {
  let teacherTime = [];
  for (let i = 0; i < teacherModule.length; i++) {
    teacherTime.push(teacherModule[i].length);
  }

  let totalTeacherTime = 0;
  for (let i = 0; i < teacherModule.length; i++) {
    let newTime = teacherTime[i].split(":");
    let minutes = parseInt(newTime[0], 10);
    let seconds = parseInt(newTime[1], 10);
    totalTeacherTime += minutes * 60 + seconds;
  }

  const avgTime = ((teacherModule.length / lessons.length) * 100).toFixed(2);

  let finalHours = Math.floor(totalTeacherTime / 3600).toFixed(0);
  let finalMinutes = ((totalTeacherTime & 3600) / 60).toFixed(0);

  return `${finalHours} hours and ${
    finalMinutes < 10 ? "0" : "" + finalMinutes
  } minutes. \nPercentage of this module is ${avgTime}% of the entire course.\n\n`;
};
